...
public class AccountServiceImpl implements AccountService {
	...
	public ApiResponse updateAccountStatus(String accountId, Status newStatus) {
		...
		if (optionalAccount.isPresent()) {
			...
			String statusMessage = newStatus == Status.ACTIVE ? AppConstant.ACCOUNT_ACTIVATED
					: AppConstant.ACCOUNT_DEACTIVATED;
	...
	public Map<String, Object> getAllAccountsByUserId(String userId) {
		...
		List<AccountResponse> accountResponses = accounts.stream().map(this::accountToAccountResponse)
	...
	public Map<String, Object> getAccountByStatusAndUserId(String userId, Status status) {
		...
		List<AccountResponse> accountResponses = activeAccounts.stream().map(this::accountToAccountResponse)
				.collect(Collectors.toList());
		response.put("accounts", accountResponses);
		return response;
	}
	...
	public Map<String, Object> updateAccountDetailsById(Account updatedDetails) {
		Map<String, Object> response = new HashMap<>();
		Account account = accountRepo.findById(updatedDetails.getId())
				.orElseThrow(() -> new BadRequestException(AppConstant.ACCOUNT_NOT_FOUND));
